...
function DateFormatter(
  ...
  switch (type) {
    case types[1]:
      if (timezone && fieldType !== 'Date') return dayjs(value).tz(timezone).format(format); // Date only value should be formatted without timezone
...
export default {
  ...
  value && timezone
  ? dayjs.tz(value, timezone).utc().format("YYYY-MM-DDTHH:mm:ss[Z]")
  : value,
  Date: (value, options) =>
